...
mtx<-as.data.frame(lung_combine_after_batch@meta.data)
mtx<-mtx[which(mtx$immune),]
for (i in 1:4) {
  ggplot(mtx[which(immune$orig.ident==c("F_1","F_2","FS_1","FS_2")[i]),], aes(x = CD4_score1, y = B_score1)) +
...
get_cor<-function(x){return(cor(mtx[which(immune$orig.ident==x),c("CD4_score1","B_score1")])[1,2])}
...
for (i in 1:4) {
  ggplot(mtx[which(immune$orig.ident==c("F_1","F_2","FS_1","FS_2")[i]),], aes(x = CD4_score1, y = cDC_marker1)) +
    geom_point(size=0.5) +                             # Scatter plot (dot plot)
    geom_smooth(method = "lm", col = "red") + # Linear regression line
    theme_minimal() +
    theme(
...
get_cor<-function(x){return(cor(mtx[which(immune$orig.ident==x),c("CD4_score1","cDC_marker1")])[1,2])}
...
wilcox.test(df3$r.ICOS[which(df3$label%in%c('F_1','F_2'))],df3$r.ICOS[which(df3$label%in%c('FS_1','FS_2'))])
wilcox.test(df3$r.CD40[which(df3$label%in%c('F_1','F_2'))],df3$r.CD40[which(df3$label%in%c('FS_1','FS_2'))])


#Figure 5E
lung_combine_after_batch$Th17_score1<-colMeans (as.data.frame(expm1(lung_combine_after_batch@assays$Spatial@data[c("Rorc","Il17a",'Ccr6','Il22','Il17f','Il23r'),]) ))
lung_combine_after_batch$Th1_score1<-colMeans(as.data.frame(expm1(lung_combine_after_batch@assays$Spatial@data[c('Ccl5','Ms4a4b','Cxcr3','Ifng',"Tbx21","Gzmk"),]) ))
lung_combine_after_batch$Th17_nonzero<-apply(as.data.frame(expm1(lung_combine_after_batch@assays$Spatial@data[c("Rorc","Il17a",'Ccr6','Il22','Il17f','Il23r'),]) ),2,function(x) sum(x>0)>=3)
lung_combine_after_batch$Th1_nonzero<-apply(as.data.frame(expm1(lung_combine_after_batch@assays$Spatial@data[c('Ccl5','Ms4a4b','Cxcr3','Ifng',"Tbx21","Gzmk"),]) ),2,function(x) sum(x>0)>=3)

# Sample data
as.data.frame(lung_combine_after_batch@meta.data[,c('Th1_score1','Th1_nonzero')])->df4
df3 <- cbind(df4,lung_combine_after_batch$orig.ident)
colnames(df3)<-c('value','nonzero','label')
as.data.frame(df3)->df3
...
summary_df <- df3 %>%
  group_by(label) %>%
  summarize(
    count_non_zero = sum(value > 0) / length(value) ,
    log_mean_value = mean(log1p(value))
  )

summary_df <- df3 %>%
  group_by(label) %>%
  summarize(
    count_non_zero = sum(nonzero) / length(value) ,
    log_mean_value = mean(log1p(value))
  )
...
ggsave('p4.png',p,width = 7,height = 4,dpi = 300)
wilcox.test(df3$value[which(df3$label%in%c('F_1','F_2'))],df3$value[which(df3$label%in%c('FS_1','FS_2'))])
wilcox.test(df3$value[which(df3$label%in%c('S_1','S_2'))],df3$value[which(df3$label%in%c('FS_1','FS_2'))])


GSE202325_ref@meta.data %>% group_by(celltype2) %>% summarize(ave=mean(nCount_RNA))->mean_count
t(as.matrix(lung_combine_after_batch@assays$conv@counts))->mtx
colnames(mtx)[13]<-'Goblet-cell'
mtx<-mtx[,gsub('_','-',mean_count$celltype2) ]
lung_combine_after_batch$nCount_Spatial/mtx%*%as.matrix(data.frame(mean_count=mean_count$ave))->lung_combine_after_batch$cell_num
colnames(df)[13]<-'Goblet-cell'
df[,colnames(mtx)]->df
apply(df, 2,function(x) x*lung_combine_after_batch$cell_num) ->df_multi
